...
app.post('/api/orders', auth, async (req, res) => {
  try {
    ...
    for (const it of items) {
      ...
      if (!prod) return res.status(400).json({ ok: false, error: 'Invalid product' });
      total += prod.price * Number(it.quantity);
    }
    const order = await Order.create({
      customerId: req.user.uid,
      products: items.map((it) => ({ productId: it.productId, quantity: Number(it.quantity), price: Number(it.price || 0) })),
      total,
    });
...
app.get('/api/orders/:customerId', auth, async (req, res) => {
  if (req.user.uid !== req.params.customerId) return res.status(403).json({ ok: false, error: 'Forbidden' });
